...
| **Supabase (claude.ai)** | connected, **active** | Targets supabase.com cloud. Our **dev** project (`efterrtpcbyjriljnfzt`) IS cloud, so the MCP works fully: `get_project_url`, `get_publishable_keys`, `execute_sql`, `get_advisors` (security/perf lints — run after every migration), `list_tables`. Schema changes still go through Drizzle migrations (checked in), not `apply_migration`. When prod moves to self-hosted Mumbai, the MCP no longer applies — use `DATABASE_URL` + drizzle-kit/psql there. | P1+ |
